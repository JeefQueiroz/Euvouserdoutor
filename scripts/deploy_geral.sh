...
echo "🚀 Iniciando reconstrução total: euvouserdoutor"

# 1. Limpeza de resíduos
rm -rf src dist vite.config.js package.json index.html .git
mkdir -p src

# 2. Criar package.json
cat <<'EOP' > package.json
{
  "homepage": "https://JeefQueiroz.github.io/Euvouserdoutor",
  "name": "euvouserdoutor",
  "private": true,
  "version": "1.0.0",
  "type": "module",
...
# 7. Criar src/App.jsx (Portal Limpo)
cat <<'EOP' > src/App.jsx
import React, { useState } from 'react';
import { Zap, Target, Send, Brain, Instagram, Award, CalendarDays, CheckSquare, Stethoscope } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#F4F7FB] font-sans text-gray-900">
      <nav className="sticky top-0 z-50 shadow-lg bg-white">
        ...
        <div className="py-5 px-10 flex justify-between items-center border-b font-sans">
        ...
        {view === 'home' ? (
          <section className="bg-[#0A192F] text-white py-32 text-center px-4">
            <div className="max-w-4xl mx-auto">
              ...
              <p className="text-xl text-blue-100 mb-12 font-light">Jefferson Queiroz | Medicina UFMG</p>
              <div className="flex flex-col sm:flex-row justify-center gap-6">
                ...
                <a href={telegram} target="_blank" rel="noreferrer" className="bg-[#2E70CE] px-10 py-4 rounded-full font-black shadow-xl hover:bg-blue-600 flex items-center justify-center gap-2 transition-all">TELEGRAM <Send size={18}/></a>
              </div>
            </div>
          </section>
        ) : view === 'materials' ? (
          <section className="py-20 px-10 max-w-6xl mx-auto grid md:grid-cols-3 gap-10">
            <div className="bg-white p-10 rounded-[40px] shadow-2xl border border-gray-100 text-center">
              ...
              <p className="text-gray-600 mb-8 font-sans">Planejamento de 100 dias focados em medicina.</p>
              <a href={telegram} target="_blank" rel="noreferrer" className="block bg-gray-900 text-white py-4 rounded-2xl font-bold">ACESSAR</a>
            </div>
            <div className="bg-white p-10 rounded-[40px] shadow-2xl border border-gray-100 text-center">
              <CheckSquare size={48} className="text-green-500 mx-auto mb-6"/>
              <h3 className="text-2xl font-black mb-4 uppercase">Simulados</h3>
              <p className="text-gray-600 mb-8 font-sans">Mais de 100 provas inéditas e FUVEST.</p>
              <a href={telegram} target="_blank" rel="noreferrer" className="block bg-green-500 text-white py-4 rounded-2xl font-bold">BAIXAR</a>
            </div>
            <div className="bg-white p-10 rounded-[40px] shadow-2xl border border-gray-100 text-center">
              <Brain size={48} className="text-pink-500 mx-auto mb-6"/>
              <h3 className="text-2xl font-black mb-4 uppercase">Flashcards</h3>
              <p className="text-gray-600 mb-8 font-sans">Método Aprovado para retenção definitiva.</p>
              <a href={telegram} target="_blank" rel="noreferrer" className="block bg-pink-500 text-white py-4 rounded-2xl font-bold">OBTER</a>
            </div>
          </section>
        ) : (
          <section className="py-40 text-center flex flex-col items-center gap-6">
            <Award size={80} className="mx-auto text-[#2E70CE] animate-bounce"/>
            <h2 className="text-5xl font-black mb-4 uppercase">Mentoria Aprovado</h2>
            <p className="text-gray-500 text-lg">O sistema de elite para futuros médicos da UFMG.</p>
          </section>
        )}
      </main>

      <footer className="bg-white py-20 border-t mt-20 text-center">
        <h2 className="text-2xl font-black tracking-tighter italic mb-4 uppercase">EUVOUSERDOUTOR</h2>
        <p className="font-bold text-gray-400">Jefferson Queiroz | Medicina UFMG</p>
        <div className="flex justify-center gap-10 mt-10">
          <a href="https://instagram.com/euvouserdoutor" target="_blank" rel="noreferrer" className="text-gray-400 hover:text-[#2E70CE] transition-colors"><Instagram /></a>
          <a href={telegram} target="_blank" rel="noreferrer" className="text-gray-400 hover:text-[#2E70CE] transition-colors"><Send /></a>
        </div>
        <p className="text-[10px] text-gray-300 mt-10 uppercase tracking-widest">© 2026 Euvouserdoutor - Todos os direitos reservados</p>
      </footer>
    </div>
  );
};
...
# 8. Instalação e Git
echo "📦 Instalando dependências e enviando para o GitHub..."
pnpm install
git init
git remote add origin https://github.com/JeefQueiroz/Euvouserdoutor
git add .
git commit -m "Restauração total do portal"
git branch -M main
...
echo "✅ TUDO CONCLUÍDO!"
echo "Confira em: https://JeefQueiroz.github.io/Euvouserdoutor"
