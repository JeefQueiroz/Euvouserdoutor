...
export const Header = ({ setView, currentView, telegram }) => (
  <header className="w-full">
    ...
    <div className="bg-white border-b border-gray-100 py-2.5 px-4 md:px-10 flex gap-6 text-[10px] font-bold uppercase tracking-widest text-gray-400 overflow-x-auto no-scrollbar">
      <span onClick={() => setView('home')} className={`cursor-pointer ${currentView === 'home' ? 'text-blue-600' : ''}`}>Home</span>
      <span onClick={() => setView('news')} className={`cursor-pointer ${currentView === 'news' || currentView === 1 ? 'text-blue-600' : ''}`}>Notícias</span>
      <span onClick={() => setView('materials')} className={`cursor-pointer ${currentView === 'materials' || currentView === 'flashcards_info' ? 'text-blue-600' : ''}`}>Materiais</span>
      <span onClick={() => setView('mentorship')} className={`cursor-pointer ${currentView === 'mentorship' ? 'text-blue-600' : ''}`}>Mentoria</span>
    </div>
    <div className="bg-gradient-to-r from-[#0A192F] to-[#2E70CE] text-white py-5 px-4 md:px-10 flex justify-between items-center sticky top-0 z-50 shadow-lg">
      ...
      <div className="flex items-center gap-4">
         <button onClick={() => setView('materials')} className="bg-white text-[#0A192F] text-[10px] font-black uppercase px-5 py-2.5 rounded-full shadow-md">BIBLIOTECA</button>
         <a href={telegram} target="_blank" rel="noreferrer" className="bg-white/10 text-white p-2 rounded-full border border-white/20"><Send size={18}/></a>
      </div>
    </div>
  </header>
);
